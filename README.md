...
- src:
    ...
    - qsession.py: Top-level module coordinating the execution of other components.
    - vaqindex.py: Main indexing component. Performs SQ indexing of data (e.g., energy calculations, non-uniform bit allocations, boundary initialization, Lloyd's algorithm), as well as binary quantization.
...
- Run sb_runner.py. The datasets/XX directory will be populated with several files shared amongst partitions (e.g. XX.ptnrvars.npz for partitioner variables), as well as partitions/0, partitions/1, ... directories containing the local quantizatio indices for each partition.
...
- Finally, the generated files should be uploaded to AWS, for use with SQLAYER
    - Run sqfiles_reduced.sh to generate a minimal zip of the files to be stored in EFS. Note that the extra files not included here are used as part of the build process.
    ...
    - A custom policy enabling Lambda functions to invoke other Lambda functions
        ...
        - "Effect": "Allow",
            - "Action": [
                ...
                - "lambda:InvokeAsync"
    - Note that more strict permissions should be applied in a production setting.
...
- Having performed these steps, now navigate to globals_qa.py in sqlayer (in Cloud9). Update self.QA_LAMBDA_ARN and self.QP_LAMBDA_ARNS with the ARNs of the newly created allocator and processor Lambdas accoridngly.
- The AWS services are now configured.
                ...
                - datasets
                    - XX
                        - partitions
                            ...
                            - ...
- Payload
    - A JSON payload is also required as input to the coordinator. A sample payload can be found in SQLAYER (sample_run_payload.json). Configure this accordingly
        ...
        - Mode = A recommended. Mode = P prints vector information.
        ...
        - check_recall: Whether to perform fine-tuning to further boost recall (recommended)
        ...
        - use_s3 = True
            - Required with above description of dataset upload. We recommend using a hybrid storage solution of S3 and EFS as described in the paper. If you want to use EFS only, then use sqfiles.sh and follow the procedure above to move that zip to EFS.
