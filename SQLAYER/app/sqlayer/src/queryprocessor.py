...
class QueryProcessor:
    ...
    def initialize(self):
        ...
        print("QA ID: ", str(self.dmg.allocator_id))
    # ----------------------------------------------------------------------------------------------------------------------------------------
    def initialize_for_query(self, qdata, pos):        
        
        # If file channel for candidates, retrieve file and populate cand array
        if (self.dmg.channel == 'F') and (pos == 0):
            fkey = qdata["candidates"]
    
            sleep_secs = 0
            file_retrieved = False
            while sleep_secs < QueryProcessor.MAX_READ_SLEEP_SECS:
                if self.dmg.receive_qp_candfile(fkey, len(self.qp_params)):
                    file_retrieved = True
                    break
                sleep_secs += 1
                print('AWAITING CHANNEL FILE : ', fkey, ' ...SLEEP COUNT ', sleep_secs)
                time.sleep(1)
                
            if not file_retrieved:
                print('UNABLE TO RETRIEVE CHANNEL FILE ', fkey, '!!')
                print('Aborting Run!')
                raise
        ...
        if self.num_candidates > 0:  
            if self.dmg.channel == 'F':
                self.candidates = self.dmg.cand_array[pos]            
            elif self.dmg.use_compression:
    ...
    def run_queries_exact(self):
        ...
        for i in range(len(self.qp_params)):
            ...
            self.initialize_for_query(json.loads(self.qp_params[i]), i)
