...
def create_query_subsets(path, fname, num_allocators, allocators_root, dup_factor, caching):
    ...
    with np.load(global_querydata_fname) as querydata:
        ...
        if dup_factor > 1 and caching == False:
            ...
            sample_inds                = np.tile(querydata['INDS']          , dup_factor)
            queries                    = np.tile(querydata['QUERIES']       , (dup_factor,1))
            predicate_sets             = np.tile(querydata['PRED_SETS']     , (dup_factor,1))
            gtraw                      = np.tile(querydata['GT_RAW']        , (dup_factor,1))
            gtraw_dists                = np.tile(querydata['GT_RAW_DISTS']  , (dup_factor,1))
            gtattr                     = np.tile(querydata['GT_ATTR']       , (dup_factor,1))
            gtattr_dists               = np.tile(querydata['GT_ATTR_DISTS'] , (dup_factor,1))             
    ...
    if dup_factor > 1 and caching == True:
        ...
        for allocator_no in range(num_allocators):
            ind_splits          [allocator_no]  = np.tile(ind_splits            [allocator_no], dup_factor),     # This tile returns a tuple, for some reason! Others don't.
            ind_splits          [allocator_no]  = ind_splits[allocator_no][0]
            query_splits        [allocator_no]  = np.tile(query_splits          [allocator_no], (dup_factor,1))
            pred_splits         [allocator_no]  = np.tile(pred_splits           [allocator_no], (dup_factor,1))
            gtraw_splits        [allocator_no]  = np.tile(gtraw_splits          [allocator_no], (dup_factor,1))
            gtraw_dists_splits  [allocator_no]  = np.tile(gtraw_dists_splits    [allocator_no], (dup_factor,1))
            gtattr_splits       [allocator_no]  = np.tile(gtattr_splits         [allocator_no], (dup_factor,1))
            gtattr_dists_splits  [allocator_no]  = np.tile(gtattr_dists_splits  [allocator_no], (dup_factor,1))
    ...
    for i in range(num_allocators):
        ...
        total_queries += split_sizes[i]
        # print('Allocator : ', i, ' Num Queries : ', split_sizes[i]*dup_factor)   
        print('Allocator : ', i, ' Num Queries : ', split_sizes[i])   
...
def main():
    ...
    # path = Path('datasets/sift1m_bb512_p10_csuint8/')
    fname = 'sift1m'    
    num_vectors = 1000000
    num_dimensions = 128
    num_attributes = 4
    ...
    # path = Path('SQUASH_MP/datasets/deep10m_bb384_p10_csuint8_30iter/')
    # fname = 'deep10m'    
